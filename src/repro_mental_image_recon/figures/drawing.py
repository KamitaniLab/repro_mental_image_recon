...
class GroupImageDrawer:
    def __init__(
        ...
        self.title_fontcolor = (
            title_fontcolor
            if isinstance(title_fontcolor, list)
            else [title_fontcolor] * len(conditions)
        )
    ...
    def _create_canvas(self, total_image_size, column_size):
        ...
        nImg_col = 1 + column_size  # since we have title column, add 1
        size_x = (
            self.image_size[0] + self.image_margin[0] + self.image_margin[2]
        ) * nImg_row + (self.group_margin[0] + self.group_margin[2]) * turn_num
        size_y = (
            self.image_size[1] + self.image_margin[1] + self.image_margin[3]
        ) * nImg_col + (self.group_margin[1] + self.group_margin[3])
        return np.ones([size_x, size_y, 3], dtype=np.uint8) * np.array(
    ...
    def _draw_images(self, image, total_image_size, column_size):
        for cind, condition in enumerate(self.conditions):
            ...
            for tind in range(total_image_size):
                ...
                image[x : (x + self.image_size[0]), y : (y + self.image_size[1]), :] = (
                    np.array(image_obj)
                )
    ...
    def _draw_image_ids(self, draw, total_image_size, column_size):
        for tind in range(total_image_size):
            row_index, column_index, turn_index = self._calculate_image_position(
                0, tind, column_size, include_condition_offset=False
            )
            x = (
                self.image_margin[0]
                + self.group_margin[0]
    ...
    def _calculate_image_position(
        self, cind, tind, column_size, include_condition_offset=True
    ):
        row_index = (
            cind + (tind // column_size) * len(self.conditions)
            if include_condition_offset
            else (tind // column_size) * len(self.conditions)
        )
    ...
    def _process_images(self, images):
        ...
        for img in images:
            if isinstance(img, str):
                ...
                raise ValueError(
                    "Image must be a filepath, ndarray, or PIL.Image instance."
                )
