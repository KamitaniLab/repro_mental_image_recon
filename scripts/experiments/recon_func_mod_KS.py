...
# Compute Loss
def compute_loss_CLIP_Shen(CLIPmodel, CLIPmodelWeight, input1, input1_type, input2, input2_type, meanCLIPfeature, cosSimilarity, similarity='corr', DEVICE=device_default):
...
def compute_loss_VGG_Shen(VGGmodel, usedLayer, layerWeight, input1, input1_type, input2, input2_type, meanVGGfeature, cosSimilarity, similarity='corr'):
...
class imageRecon():
    ...
    def withoutLangevin(self, initInput=None, initInputType='PIL', optimizer='Adam', lr=0.5, T=1.0, numReps=None, returnVec=False):
        ...
        for index_rep in tqdm(range(numReps), ncols=100, desc="Progress rate"):  # range(numReps)
            # add for reduce randomness
            torch.manual_seed(self.seed + index_rep)
            np.random.seed(self.seed + index_rep)
