...
def fixed_image(dev, size=224):
    ...
    return torch.linspace(-1.0, 1.0, 3 * size * size, device=dev).reshape(
        1, 3, size, size
    )
...
def build_recon(args, dev):
    ...
    VQGAN = model_loading.load_vqgan(
        cfg, ckpt_path=tt + "/logs/vqgan_imagenet_f16_1024/checkpoints/last.ckpt"
    ).to(dev)
    VQGAN.eval()
    ...
    def load_dec(model, layer):
        with open(decpath(model, layer), "rb") as f:
            dt = pickle.load(f)
        return (
            torch.tensor(dt[0].astype("float32"), dtype=torch.float32)
            .to(dev)
            .unsqueeze(0)
        )
    ...
    return dict(
        args=(
            [load_dec("VGG19", name) for name in vgg_dirs],
...
def work_recon(out_path, args, dev):
    ...
    for recImg, step, lv, lc, vec in recon.withoutLangevin(
        ...
        img_sgd = recImg
        print(
            f"  [SGD] {step}/{b['n_sgd']} loss_VGG={lv:.6f} loss_CLIP={lc:.6f} "
            f"{time.time() - t0:.0f}s",
            flush=True,
        )
        if vec is not None:
    ...
    if b["n_lang"] > 0:
        ...
        for recImg, step, lv, lc, vec in recon.Langevin(
            ...
            img_final = recImg
            print(
                f"  [Langevin] {step}/{b['n_lang']} loss_VGG={lv:.6f} "
                f"loss_CLIP={lc:.6f} {time.time() - t0:.0f}s",
                flush=True,
            )
    ...
    np.savez(
        out_path,
        snaps=np.stack(recon.snapshots),
        steps=np.array(recon.snap_steps),
        n_sgd=b["n_sgd"],
    )
...
def compare_npy(a_path, b_path):
    ...
    d = np.abs(a - b)
    print(
        f"    sha A={hashlib.sha256(a.tobytes()).hexdigest()[:16]}  "
...
def compare_recon(a_path, b_path):
    ...
    print(
        f"    {'step':>10}  {'phase':>8}  {'status':>9}  {'max|d|':>11}  "
        f"{'rel':>10}  {'corr':>14}"
    )
    all_same = True
    for i in order:
...
def drive(args):
    ...
    tag = dev.type
    print(
        f"host={os.uname().nodename}  device={dev}  gpu={gpu}  "
    ...
    if args.recon:
        ...
        print(
            f"    images: {out_dir}/{tag}_recon1_sgd.png, "
...
def main():
    ...
    p.add_argument(
        "--recon", action="store_true", help="also run the real reconstruction (slow)"
    )
    p.add_argument(
        "--seed",
        type=int,
    ...
    p.add_argument(
        "--n-lang", type=int, default=None, help="override numReps_withLangevin"
    )
