...
def recon_path(recon_root, method, subj, roi, stim_id):
    return os.path.join(
        recon_root, method, subj, roi, f"recon_img_normalized-Img{stim_id:04d}.jpg"
    )
...
def main():
    ...
    ap.add_argument(
        "--summary_dir", required=True, help="dir with distances_{metric}.csv"
    )
    ap.add_argument(
        "--recon_root",
    ...
    ap.add_argument(
        "--also_farthest", action="store_true", help="also render the farthest pairs"
    )
    ap.add_argument(
        "--per_subject",
        action="store_true",
    ...
    ap.add_argument(
        "--rank_by",
        default="z_col",
        choices=["distance", "z_col", "ident_col"],
    ...
    label = {
        ...
        "z_col": "difficulty-normalized z",
        "ident_col": "per-image identification",
    }[args.rank_by]
    out_tag = {"distance": "closest", "z_col": "bestz", "ident_col": "bestid"}[
        args.rank_by
    ]
        ...
        for subj, grp in groups:
            ...
            if args.also_farthest:
                render(
                    ...
                    args.roi,
                    os.path.join(
                        args.summary_dir, f"examples_{out_tag}_worst_{metric}{sfx}"
                    ),
                )
