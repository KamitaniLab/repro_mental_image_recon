...
def load_preference_summary(prefix: str, recon_root: Path) -> dict[str, dict[str, np.ndarray]]:
...
def main() -> None:
    ...
    for label, (model, key) in GROUP_KEYS.items():
        ...
        print(f"  {label:<15s} {k:>2d}/{n} wins, {p_hat:.3f} "
              f"[{wl:.3f}, {wu:.3f}] Wilson  [{cl:.3f}, {cu:.3f}] CP,  p = {p:.3f}")
        rows.append(dict(figure="Fig5E", metric=label, condition=TWO_METHOD_LABELS[0],
                         wins=k, n=n, proportion=p_hat, wilson_lo=wl, wilson_hi=wu,
                         cp_lo=cl, cp_hi=cu, p_value=p, null_p=0.5))
    ...
    print("       CI = 95% Clopper-Pearson; p = two-sided exact binomial vs chance 0.25")
    print("=" * 78)
    for label, (model, key) in GROUP_KEYS.items():
        ...
        for m, name in enumerate(FOUR_METHOD_LABELS):
            ...
            print(f"      {name:<20s} {k:>2d}/{n} best, {p_hat:.3f} "
                  f"[{cl:.3f}, {cu:.3f}],  p = {p:.3f}")
            rows.append(dict(figure="Fig5D", metric=label, condition=name,
                             wins=k, n=n, proportion=p_hat, wilson_lo=wl, wilson_hi=wu,
                             cp_lo=cl, cp_hi=cu, p_value=p, null_p=0.25))
