...
def build_dreamsim(device):
    ...
    def dist(a, b):  # PIL, PIL -> float distance
        with torch.no_grad():
            return float(model(preprocess(a).to(device), preprocess(b).to(device)).cpu())
    return dist
...
def build_lpips(device):
    ...
    def dist(a, b):
        with torch.no_grad():
            return float(net(tf(a).unsqueeze(0).to(device), tf(b).unsqueeze(0).to(device)).cpu())
    return dist
...
def summarize(matched, mismatched):
    ...
    return {
        ...
        "matched_mean": float(matched.mean()), "matched_std": float(matched.std()),
        "matched_median": float(np.median(matched)),
        "mismatched_mean": float(mismatched.mean()), "mismatched_std": float(mismatched.std()),
        "mismatched_median": float(np.median(mismatched)),
        "auc_identification": float(auc), "mannwhitney_p": float(p),
    }
...
def main():
    ...
    ap.add_argument("--id_min", type=int, default=None, help="only stim ids >= this (e.g. 17 for natural)")
    ap.add_argument("--out_dir", default=None)
    ap.add_argument("--mark_subject", default=None, help="subject to highlight stimuli on (e.g. S2)")
    ap.add_argument("--mark_ids", default=None, help="comma stim ids to mark on the mark_subject hist")
    args = ap.parse_args()

    out_dir = args.out_dir or os.path.join(args.recon_root, args.method, "distance_summary")
    ...
    for subj in subjects:
        ...
        ids = sorted(i for i in rec_idx if i in src_idx and (args.id_min is None or i >= args.id_min))
    ...
    for metric in metrics:
        ...
        for subj in subjects:
            ...
            for k, i in enumerate(ids):
                row_off = np.delete(M[k], k)          # recon_i vs other sources (controls recon)
                col_off = np.delete(M[:, k], k)       # other recons vs source_i (controls source difficulty)
                z_col = float((diag[k] - col_off.mean()) / (col_off.std() + 1e-9))
                z_row = float((diag[k] - row_off.mean()) / (row_off.std() + 1e-9))
                ident_col = float(np.mean(diag[k] < col_off))   # matched recon beats distractor recons at source_i
                ident_row = float(np.mean(diag[k] < row_off))   # matched source beats distractor sources for recon_i
                rows.append((subj, i, float(diag[k]), float(row_off.mean()), float(col_off.mean()),
                             float(col_off.std()), z_col, z_row, ident_col, ident_row))
        overall = summarize(matched_all, mismatched_all)
        results[metric] = {"overall": overall, "per_subject": per_subject,
                           "per_subject_raw": per_subject_raw,
                           "matched": np.asarray(matched_all), "mismatched": np.asarray(mismatched_all)}
        # write per-image CSV
        csv = os.path.join(out_dir, f"distances_{metric}.csv")
        with open(csv, "w") as f:
            f.write("subject,stim_id,matched_distance,row_null_mean,col_null_mean,"
    ...
    for ax, metric in zip(axes, metrics):
        ...
        ax.hist(mm, bins=bins, density=True, alpha=0.5, color="0.6",
                label=f"mismatched (null, n={len(mm)})")
        ax.hist(m, bins=bins, density=True, alpha=0.6, color="#c0392b",
        ...
        ax.set_title(f"{metric}  (AUC={ov['auc_identification']:.2f}, "
                     f"p={ov['mannwhitney_p']:.1e})", fontsize=10)
    ...
    fig.suptitle(f"Reconstruction-target distance: {tag}, subjects={','.join(subjects)}",
                 fontsize=11)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(out_dir, f"distance_distribution_{tag}.{ext}"), dpi=200)
    plt.close(fig)

    # ---- figure: per-subject histograms (rows = subjects, cols = metrics) ----
    nrow = len(subjects)
    fig, axes = plt.subplots(nrow, len(metrics), figsize=(5.2 * len(metrics), 2.6 * nrow),
                             squeeze=False)
    for ci, metric in enumerate(metrics):
        ...
        for ri, subj in enumerate(subjects):
            ...
            ax.set_title(f"{subj} — {metric}  (AUC={s['auc_identification']:.2f}, "
                         f"p={s['mannwhitney_p']:.1e})", fontsize=9)
            if ri == nrow - 1:
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(out_dir, f"distance_distribution_persubject_{tag}.{ext}"), dpi=200)
    plt.close(fig)
    ...
    if args.mark_subject and args.mark_ids:
        ...
        fig, axes = plt.subplots(len(metrics), 1, figsize=(7.2, 3.0 * len(metrics)), squeeze=False)
        for mi, metric in enumerate(metrics):
            ax = axes[mi][0]
            m, mm = results[metric]["per_subject_raw"][subj]
            lo, hi = min(m.min(), mm.min()), max(m.max(), mm.max())
            bins = np.linspace(lo, hi, 26)
            ax.hist(mm, bins=bins, density=True, alpha=0.5, color="0.6", label="null (other stimuli)")
            ax.hist(m, bins=bins, density=True, alpha=0.55, color="#c0392b", label="matched (all)")
            ymax = ax.get_ylim()[1]
            ...
            for j, sid in enumerate(mark_ids):
                ...
                ax.text(d, ymax * (0.96 - 0.13 * j), f" #{sid} {names.get(sid,'')}\n d={d:.3f}",
                        color=col, fontsize=8, ha="left", va="top", rotation=0)
            s = results[metric]["per_subject"][subj]
            ax.set_title(f"{subj} — {metric}  (matched mean={s['matched_mean']:.3f}, "
            ...
            for sp in ("top", "right"):
                ax.spines[sp].set_visible(False)
        fig.suptitle(f"Marked stimuli on {subj} distance distribution: {tag}", fontsize=11)
        fig.tight_layout(rect=(0, 0, 1, 0.97))
        for ext in ("pdf", "png"):
            fig.savefig(os.path.join(out_dir, f"distance_marked_{subj}_{tag}.{ext}"), dpi=200)
        plt.close(fig)
    ...
    fig.legend(handles=[Patch(facecolor=C_N, alpha=0.55, label="mismatched (null, other stimuli)"),
    ...
    fig.suptitle(f"Matched vs null distance: {tag}, subjects={','.join(subjects)}",
                 fontsize=11, y=1.06)
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(out_dir, f"distance_box_{tag}.{ext}"), dpi=200, bbox_inches="tight")
    plt.close(fig)

    # ---- text summary ----
    with open(os.path.join(out_dir, f"summary_{tag}.txt"), "w") as f:
        f.write(f"method={args.method} subjects={subjects} roi={args.roi} "
                f"id_min={args.id_min} source={args.source_dir}\n\n")
        for metric in metrics:
            ov = results[metric]["overall"]
            f.write(f"== {metric} ==\n")
            f.write(f"  matched    : mean={ov['matched_mean']:.4f} sd={ov['matched_std']:.4f} "
                    f"median={ov['matched_median']:.4f} (n={ov['n_matched']})\n")
            f.write(f"  mismatched : mean={ov['mismatched_mean']:.4f} sd={ov['mismatched_std']:.4f} "
                    f"median={ov['mismatched_median']:.4f} (n={ov['n_mismatched']})\n")
            f.write(f"  AUC(identification)={ov['auc_identification']:.4f}  "
                    f"Mann-Whitney U p(matched<mismatched)={ov['mannwhitney_p']:.3e}\n")
            for subj, s in results[metric]["per_subject"].items():
                f.write(f"    {subj}: matched={s['matched_mean']:.4f} "
