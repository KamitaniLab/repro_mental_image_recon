...
def convertImageIntoCLIPinput(Image):
    ...
    CLIPinput = torch.tensor(
        CLIPinput.astype(np.float32).transpose(2, 0, 1)[np.newaxis]
    )
...
def calculate_similairty_koidemajima(
    reconImg,
    targetImg,
    meanVGGfeature_,
    meanCLIPfeature_,
    ...
    if len(used_layers_VGG__in) != 0:
        loss_VGG = recon_func.compute_loss_VGG(
            VGGmodel_,
            used_layers_VGG__in,
            VGGlayerWeight_,
            recon_VGG_input,
...
def perform_evaluation(result_dir, reconMethod="original"):
    ...
    for mi in range(len(CLIPmodel_)):
        x = scipy.io.loadmat(
            os.path.join(
                meanFeatureDir, CLIP_modelNames[mi], CLIP_usedLayer, "meanFeature_.mat"
            )
        )
    ...
    for subject in subject_list:
        ...
        for i, t_img in enumerate(target_image_pil_list):
            for j, r_img in enumerate(recon_image_pil_list):
                similarity = calculate_similairty_koidemajima(
                    r_img,
                    t_img,
                    meanVGGfeature_,
                    meanCLIPfeature_,
    ...
    # sim_matrixの保存
    save_name = (
        "pairwise_identification_results_koide_majima_optsame_metric_sim_matrix.pkl"
    )
    with open(os.path.join(save_base_dir, save_name), "wb") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_dir", type=str, default="results/rep_recon_image_koide-majima"
    )
