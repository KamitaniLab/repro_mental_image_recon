...
def main():
    ...
    print(
        f"{len(conditions)} conditions x {args.runs} runs, seed={args.seed}  "
        f"(n_sgd={n_sgd}, n_lang={n_lang})",
        flush=True,
    )
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for k, (subj, tid) in enumerate(conditions, 1):
            ...
            for _ in range(args.runs):
                img, lat = run_once(
                    models, feats, dt_cfg, args.method, seed, dev, n_sgd, n_lang
                )
                imgs.append(img)
                lats.append(lat)
            ...
            el = time.time() - t0
            spread = (
                f"  (over {len(means)} comparisons: {min(means):.3f}-{max(means):.3f})"
                if len(means) > 1
                else ""
            )
            print(
                f"[{k}/{len(conditions)}] {subj} target{tid:02d} seed={seed}  "
    ...
    v = np.array(vals)
    print(
        f"mean|d| over {len(v)} pairs: mean={v.mean():.3f}  "
