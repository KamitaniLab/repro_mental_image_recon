...
for a_path in a_pkls:
    ...
    if np.array_equal(va, vb):
        ...
        print(f'    latent_vec: DIFFERS  max|d|={d.max():.3e}  '
    ...
    for k in ('loss_vgg_withLangevin_list', 'loss_clip_withLangevin_list'):
        ...
        tag = 'IDENTICAL' if np.array_equal(la, lb) else \
              f'DIFFERS max|d|={np.abs(la-lb).max():.3e}'
        print(f'    {k}: {tag}')
...
for a_path in a_jpgs:
    ...
    d = np.abs(ia - ib)
    status = 'IDENTICAL' if d.max() == 0 else f'DIFFERS max={d.max()} mean={d.mean():.4f}'
    print(f'--- {rel}\n    image (0-255): {status}')
    if d.max() != 0:
        all_identical = False

print()
print('RESULT:', 'fully reproducible (bit-exact)' if all_identical
