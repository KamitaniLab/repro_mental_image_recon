...
def run_recon(
    ...
    save_subject_list = [f"S{int(s[1:])}" if s[1:].isdigit() else s for s in subjects]
    subject_dict = dict(zip(subjects, save_subject_list))

    def _keep_step(step_idx):
        return save_trajectory and (
            trajectory_stride <= 1 or (step_idx % trajectory_stride == 0)
        )
...
def make_tag(params):
    return (
        f"lr_a{fmt(params['lr_a'])}_lr_b{fmt(params['lr_b'])}"
...
def build_oat(args):
    ...
    grid_keys = GRID_KEYS
    baseline = {
        "lr_a": args.baseline_lr_a,
        "lr_b": args.baseline_lr_b,
        "lr_gamma": args.baseline_lr_gamma,
        "T": args.baseline_T,
...
DEFAULT_OUT = {
    "oat": "./results/oat_sampling_params",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr_a", type=float, nargs="+", default=[0.00015, 0.01, 0.1, 1.0, 10]
    )
    parser.add_argument(
        "--lr_b", type=float, nargs="+", default=[0.05, 0.115533, 0.15, 0.5, 1.0]
    )
    parser.add_argument(
        "--lr_gamma", type=float, nargs="+", default=[0, 0.03, 0.055, 0.1, 0.3]
    )
    parser.add_argument(
        "--T", type=float, nargs="+", default=[1e-6, 1e-4, 1e-2, 0.1, 1.0]
    )
    parser.add_argument(
        "--numReps_withLangevin", type=int, nargs="+", default=[0, 250, 500, 1000, 1500]
    )
    parser.add_argument(
        "--numReps_withoutLangevin", type=int, nargs="+", default=[0, 1000]
    )
    ...
    # scope
    parser.add_argument(
        "--subjects", type=str, nargs="+", default=["S01", "S02", "S03"]
    )
    parser.add_argument(
        "--targetID",
        type=int,
        nargs="+",
        default=list(range(25)),
    ...
    parser.add_argument(
        "--out", type=str, default=None, help="output root (default: depends on --mode)"
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
    ...
    print(
        f"[shard {args.shard_id}/{args.num_shards}] handling {len(my_indices)}/{total} combos "
    ...
    if args.dry_run:
        ...
        print(
            f"[dry-run] {len(my_indices)} combos for this shard "
    ...
    for n, idx in enumerate(my_indices):
        ...
        print(
            f"[shard {args.shard_id}] [{n + 1}/{len(my_indices)}] (global {idx}/{total}) tag={tag}"
        )
