...
def clip_crop_matrix(enc, recs, srcs, device, ncrop=NUM_CROP_EVAL):
    ...
    with torch.no_grad():
        for r in recs:
            crops = recon_func.createCrops(
                _clip_tf(r).unsqueeze(0).to(device), ncrop, DEVICE=device
            )
...
def load_pairs(out_dir):
    ...
    return [Image.open(p).convert("RGB") for p in src], [
        Image.open(p).convert("RGB") for p in rec
    ]
...
ALEX_RAND_SEED = (
    1234  # must match recovery_matrix_invert.py so the random AlexNet is identical
)
...
def build_encoder(name, device):
    ...
    if name in (
        ...
        def sim(rec, src):
            with torch.no_grad():
                fr = (
                    model.features[:sl](tf(rec).unsqueeze(0).to(device))
                    .reshape(1, -1)
                    .float()
                )
                fs = (
                    model.features[:sl](tf(src).unsqueeze(0).to(device))
                    .reshape(1, -1)
                    .float()
                )
            return pearson(fr, fs)
    ...
    if name == "lpips":
        ...
        def sim(rec, src):
            with torch.no_grad():
                d = net(
                    tf(rec).unsqueeze(0).to(device), tf(src).unsqueeze(0).to(device)
                )
            return -float(d.cpu())  # similarity = -distance
...
def main():
    ...
    ap.add_argument(
        "--clip_aug",
        action="store_true",
        help="CLIP eval with crop aug (default: single image, no aug)",
    )
    ap.add_argument(
        "--eval_crops",
        type=int,
        default=NUM_CROP_EVAL,
        help="num crops for CLIP-aug eval",
    )
    ap.add_argument(
        "--evaluators",
        default=",".join(ALL_EVALUATORS),
    ...
    with open(
        os.path.join(args.out_dir, "recovery_check_identification.pkl"), "wb"
    ) as f:
