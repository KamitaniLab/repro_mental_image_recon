...
def _plot_violin(
    ax, per_subject: dict[str, np.ndarray], ylabel: str, log: bool
) -> None:
    ...
    parts = ax.violinplot(
        plot_data, positions=positions, widths=0.8, showmedians=True, showextrema=False
    )
    ...
    if log:
        ...
        if dropped:
            ax.annotate(
                f"{dropped} zero-SD coords omitted (log)",
                xy=(0.02, 0.02),
...
def _draw_autocorr_curves(ax, lags, per_subject_curves):
    ...
    for subject, curves in per_subject_curves.items():
        ...
        ax.plot(
            lags,
            mean_curve,
            color=SUBJECT_COLORS.get(subject, "0.3"),
            linewidth=1.3,
            label=subject,
        )
...
def _plot_autocorr(
    ax,
    lags: np.ndarray,
    per_subject_curves: dict[str, np.ndarray],
    title: str,
    ...
    ax.set_title(title)
    ax.annotate(
        f"max pooled SD = {pooled_sd.max():.4f}",
        xy=(0.03, 0.06),
        xycoords="axes fraction",
        fontsize=7,
        color="0.4",
    ...
    if zoom_window is not None:
        ...
        lo_y = float(
            min(subject_means[:, window].min(), (pooled_mean - pooled_sd)[window].min())
        )
        hi_y = float(
            max(subject_means[:, window].max(), (pooled_mean + pooled_sd)[window].max())
        )
...
def _make_figure(
    pixel_std_all, latent_std_all, pixel_ac, latent_ac, lags, n_total, log: bool
):
    fig, axes = plt.subplots(2, 2, figsize=(10, 8))
    # latent on the left column, pixel on the right column
    _plot_violin(axes[0, 0], latent_std_all, "Per-latent SD over SGLD chain", log=log)
    ...
    _plot_autocorr(
        axes[1, 0], lags, latent_ac, "Latent autocorrelation", zoom_window=(28, 30)
    )
    _plot_autocorr(axes[1, 1], lags, pixel_ac, "Pixel autocorrelation")

    scale = "log y" if log else "linear y"
    fig.suptitle(
        f"SGLD sampling effect across all subjects x stimuli "
...
def export(
    summary_dir: Path, output_dir: Path, table_dir: Path, subjects: tuple[str, ...]
) -> None:
    ...
    n_total = sum(
        int(np.isfinite(d["pixel_std_mean"]).sum()) for d in summaries.values()
    )

    ensure_directory(output_dir)
    for log, suffix in ((False, ""), (True, "_log")):
        fig = _make_figure(
            pixel_std_all, latent_std_all, pixel_ac, latent_ac, lags, n_total, log=log
        )
    ...
    rows = _summary_rows("pixel_per_coord_SD", pixel_std_all) + _summary_rows(
        "latent_per_coord_SD", latent_std_all
    )
