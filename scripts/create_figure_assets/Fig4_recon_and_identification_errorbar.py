...
DIAG_EVAL = {
    ...
    "openclip_laion": "clip_laion",
    "alexnet_conv5": "alexnet_conv5",
    "alexnet_rand_conv5": "alexnet_rand_conv5",
}
MM = 1.0 / 25.4
...
def _load_accs(
    base, sp
):  # base/<opt_space>/recovery_check_identification.pkl -> [acc per EVAL]
    d = pickle.load(
        open(os.path.join(base, sp, "recovery_check_identification.pkl"), "rb")
    )
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=190.0,
        help="total figure width (A4=210, with margins=190)",
    )
    ap.add_argument("--fontsize", type=float, default=10.0)
    ap.add_argument(
        "--bar_ratio", type=float, default=3.3, help="bar panel width / image cell"
    )
    args = ap.parse_args()
    fs = args.fontsize
    ...
    if args.out is None:
        args.out = str(
            ensure_directory(project_root() / "assets" / "fig04")
    ...
    cols = sorted(
        ...
        .choice(n_imgs, args.n_cols, replace=False)
        .tolist()
    )
    ...
    for sp in OPT_ROWS:
        rep_dirs = sorted(
            d
            for d in glob.glob(os.path.join(args.reps_root, "rep*"))
            if os.path.exists(os.path.join(d, sp, "recovery_check_identification.pkl"))
        )
        if not rep_dirs:
            raise SystemExit(
                f"no rep*/{sp}/recovery_check_identification.pkl under {args.reps_root}"
            )
        ...
        n_reps[sp] = A.shape[0]
        print(
            f"[{sp}] n_reps={A.shape[0]}  diag mean={accs[sp][EVALS.index(DIAG_EVAL[sp])]:.3f} "
    ...
    def add_ax(left_in, bottom_in, w_in, h_in):
        return fig.add_axes(
            [left_in / fig_w, bottom_in / fig_h, w_in / fig_w, h_in / fig_h]
        )
    ...
    for r, row in enumerate(ROWS):
        ...
        bx.text(
            diag_i,
            accs[row][diag_i] + errs[row][diag_i] + 0.02,
            f"{accs[row][diag_i]:.2f}",
            ha="center",
            va="bottom",
            fontsize=fs,
            color="#c0392b",
    ...
    print(
        f"saved: {args.out}.pdf / .png  ({fig_w / MM:.0f}x{fig_h / MM:.0f} mm), cols={cols}, "
