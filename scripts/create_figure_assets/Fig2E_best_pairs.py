...
def main():
    ...
    ap.add_argument(
        "--subject", default=None, help="subject to plot (default: all rows)"
    )
    ap.add_argument(
        "--top",
        type=int,
    ...
    ap.add_argument(
        "--metric_label",
        default=None,
        help="distance name shown in titles (default: from CSV name)",
    )
    ap.add_argument(
        "--width_mm",
        type=float,
        default=210.0,
        help="figure width in mm (210 = A4 width)",
    )
    ap.add_argument(
        "--font_pt",
        type=float,
    ...
    metric = args.metric_label or os.path.splitext(os.path.basename(args.csv))[
        0
    ].replace("distances_", "")
    ...
    if missing:
        raise SystemExit(
            f"[error] no target image for stimulus ids {missing} "
    ...
    if len(best) + len(worst) > len(rows):
        raise SystemExit(
            f"[error] --top {n_top} + --bottom {args.bottom} exceeds {len(rows)} pairs"
        )
    print(
        f"[info] {args.subject or 'all'}: best-{len(best)}"
        + (f" / worst-{len(worst)}" if worst else "")
        + f" by {metric}"
    )
    for r in best + worst:
    ...
    cap_h = (
        (FS_HEAD * 1.3 + 4) / 72 if (worst and len(chunks) == 1) else 0.0
    )  # block captions
    ...
    for k, chunk in enumerate(chunks):
        y_top = (
            head_h + cap_h + k * block_h + title_h
        )  # inches from figure top to the target row
        for j, r in enumerate(chunk):
            ...
            for row_i, p in enumerate((source_path, recon_path)):
                ...
                if j == 0:
                    ax.set_ylabel(
                        "Target" if row_i == 0 else "Reconstruction",
                        fontsize=FS,
                        rotation=90,
                        va="center",
                        ha="center",
    ...
    if cap_h:  # captions over the best / worst blocks (single-row layout only)
        blocks = (
            ("closest", 0, len(best) - 1),
            ("farthest", len(best) + 1, len(items) - 1),
        )
    ...
    tag = ("all" if args.top <= 0 else f"top{len(best)}") + (
        f"_bottom{len(worst)}" if worst else ""
    )
